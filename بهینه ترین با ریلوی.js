...
async function checkAutoResets(env, ctx) {
	...
	try {
		const cache = caches.default;
		const cacheReq = new Request("https://internal.makevaslim/auto_reset");
...
async function checkAutoRotates(env, ctx) {
	...
	try {
		const cache = caches.default;
		const cacheReq = new Request("https://internal.makevaslim/auto_rotate");
		if (await cache.match(cacheReq)) return;
...
export default {
	async fetch(request, env, ctx) {
		...
		if (Router.isWebSocketUpgrade(request) && url.pathname.startsWith("/p/")) {
			const perIpProxy = extractPerIpProxyFromPath(url.pathname);
			if (perIpProxy) {
				return await Router.handleWebSocket(request, env, ctx, perIpProxy);
			}
			return await Router.handleWebSocket(request, env, ctx);
		}

		if (Router.isWebSocketUpgrade(request) && (url.pathname === "/Ma_Ke_Vaslim" || url.pathname.startsWith("/Ma_Ke_Vaslim/") || url.pathname.includes("Ma_Ke_Vaslim"))) {
			let perIpProxy = extractPerIpProxyFromPath(url.pathname);
			return await Router.handleWebSocket(request, env, ctx, perIpProxy);
		}
...
function extractPerIpProxyFromPath(pathname) {
	if (!pathname || !pathname.startsWith("/p/")) return null;
	try {
		const parts = pathname.split("/").filter(p => p.length > 0);
		if (parts.length < 2) return null;
		let proxyB64 = parts[1];
		let b64 = proxyB64.replace(/-/g, '+').replace(/_/g, '/');
	...
	async handleApi(request, url, env, ctx) {
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			try {
				...
				const githubRes = await fetch("https://raw.githubusercontent.com/mkvaslim44/Ma_ke_vaslim-Panel/main/panel-source?t=" + Date.now(), {
					headers: {
	...
	async generateText(user, host) {
		...
		rawIpList.forEach((item) => {
			...
			if (proxyB64) {
				...
				const directLink = generateConfig(user.uuid, ipOrDomain, port, remarkBase, fp, currentFragment, extraParams, null);
			...
			if (proxyB64) {
				...
				links.push(proxyLinkGrpc);
			} else {
				const directLinkGrpc = generateGrpcConfig(user.uuid, ipOrDomain, port, remarkGrpcBase + " 🚀", fp, null, extraParamsGrpc);
				links.push(directLinkGrpc);
			}
...
async function handleVLESSGrpc(env, storedData = null, ctx = null, request = null) {
	...
	function createGrpcFrame(data) {
		const chunk = convertToUint8Array(data);
		if (!chunk.byteLength) return new Uint8Array(0);
		const frame = new Uint8Array(5 + chunk.byteLength);
		frame[0] = 0;
		new DataView(frame.buffer).setUint32(1, chunk.byteLength, false);
		frame.set(chunk, 5);
		return frame;
	}
	...
	const grpcTask = (async () => {
		try {
			...
			while (true) {
				...
				if (!value || value.byteLength === 0) continue;
				await addBytes(value.byteLength);
				currentGrpcBuffer = concatBytes(currentGrpcBuffer, value);
				while (currentGrpcBuffer.byteLength >= 5) {
					...
					if (currentGrpcBuffer.byteLength < 5 + msgLen) break;
					const payload = currentGrpcBuffer.slice(5, 5 + msgLen);
					currentGrpcBuffer = currentGrpcBuffer.slice(5 + msgLen);
					...
					if (!isHeaderParsed) {
						...
						try {
							let offset = 17;
							const optLen = payload[offset++];
							offset += optLen;
							const cmd = payload[offset++];
							const port = (payload[offset++] << 8) | payload[offset++];
							const addrType = payload[offset++];
							let addr = "";
							if (addrType === 1) {
								addr = `${payload[offset++]}.${payload[offset++]}.${payload[offset++]}.${payload[offset++]}`;
							} else if (addrType === 2) {
								const domainLen = payload[offset++];
								addr = GLOBAL_DECODER.decode(payload.slice(offset, offset + domainLen));
								offset += domainLen;
							} else if (addrType === 3) {
								const v6 = [];
								for (let i = 0; i < 8; i++) v6.push(((payload[offset++] << 8) | payload[offset++]).toString(16));
								addr = v6.join(":");
							}
							const rawData = payload.slice(offset);
							const respHeader = new Uint8Array([payload[0], 0]);
								...
								if (port === 53) {
									try {
										...
										if (r.value) {
											await addBytes(r.value.byteLength);
											const merged = concatBytes(respHeader, r.value);
											const frame = createGrpcFrame(merged);
								...
								(async () => {
									try {
										...
										while (true) {
											...
											const frame = createGrpcFrame(toSend);
											await responseWriter.write(frame);
										}
									} catch (e) {}
									try {
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                ...
                <div class="flex items-center gap-3 bg-gray-100 dark:bg-zinc-800/60 px-3 py-1.5 rounded-full border border-gray-200 dark:border-zinc-800/80 shadow-sm flex-shrink-0 w-fit">
                    ...
                    <a href="https://t.me/makevaslim4" target="_blank" rel="noopener noreferrer" class="text-emerald-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="Bot">
                    ...
                    <div class="flex flex-col gap-2 pt-4 lg:pt-0 border-t-2 lg:border-t-0 border-gray-300 dark:border-amoled-border justify-between">
                        <div class="flex flex-col flex-1">
                            ...
                            <div class="grid grid-cols-2 gap-2 mb-2 w-full">
                                ...
                                <a href="https://github.com/mkvaslim44/Ma_ke_vaslim-Panel#%EF%B8%8F-build-your-own-socks5-proxy-zeus-relay" target="_blank" class="text-[11px] bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-2 rounded border border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition font-black shadow-sm text-center whitespace-nowrap">ساخت پـروکـسـی شخصی</a>
                            </div>
...
<div id="update-modal" class="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <div class="space-y-3">
            ...
            <a href="https://t.me/makevaslim4" target="_blank" class="w-full py-3.5 bg-orange-50 dark:bg-orange-950/30 hover:bg-orange-100 dark:hover:bg-orange-900/50 text-orange-600 dark:text-orange-500 border border-orange-300 dark:border-orange-500 font-bold rounded-md text-sm transition duration-300 shadow-sm flex items-center justify-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function setModalState(modalId, show) {
	...
	async function checkGlobalMessage() {
		...
		links.push('vle' + 'ss://' + (user.uuid || '') + '@0.0.0.0:1?encryption=none&security=none&type=ws&host=' + host + '&path=' + dynPath + '#' + encodeURIComponent(m2));
            ...
            const sniDomain = "makv.cc.cd";
            function encodeProxyForPathUI(proxyStr) {
                if (!proxyStr) return null;
                try { return btoa(encodeURIComponent(proxyStr)).replace(/=/g, '').replace(/\+/g, '-').replace(/\//g, '_'); } catch(e) { return null; }
            }
            ips.forEach((ipItem) => {
                let cleanIp = ipItem;
                let perIpProxy = null;
                if (ipItem.includes("|")) {
                    const partsPipe = ipItem.split("|");
                    cleanIp = partsPipe[0].trim();
                    perIpProxy = partsPipe[1].trim();
                }
                let ipOrDomain = cleanIp;
                let portFromIp = null;
                let customName = user.username;
                if (cleanIp.includes("#")) {
                    const [addrPart, name] = cleanIp.split("#");
                    customName = name || user.username;
                    if (addrPart.includes(":")) {
                        const pp = addrPart.split(":");
                        ipOrDomain = pp[0];
                        portFromIp = pp[1];
                    } else ipOrDomain = addrPart;
                } else if (cleanIp.includes(":")) {
                    const pp = cleanIp.split(":");
                    if (pp.length === 2 && !isNaN(pp[1])) {
                        ipOrDomain = pp[0];
                        portFromIp = pp[1];
                    } else ipOrDomain = cleanIp;
                }
                if (!ipOrDomain) return;
                const portsToUse = portFromIp ? [portFromIp] : ports;
                portsToUse.forEach((portStr) => {
